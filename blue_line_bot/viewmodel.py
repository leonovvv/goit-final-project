...
VALUE_ERROR_MESSAGES = {
    ...
    "Phone.NotNumeric": "Phone must be numeric",
    "Phone.LengthMustBe10": "Phone lenght must be 10 digits",
    "Birthday.Invalid": "Birthday should be in format d.m.Y (e.g. 27.1.2000)",
    "Email.Required": "Email can not be empty",
    "Email.Invalid": "Input a valid email",
    "Address.Required": "Address can not be empty",
    "Record.PhoneDuplicate": "This phone already exists in the record",
    "Record.PhoneNotFound": "This phone does not exist in the record",
...
def error_decorator(func):
    def inner(*args, **kwargs):
        try:
            ...
            error_code = str(ex)
            error_message = VALUE_ERROR_MESSAGES.get(
                error_code,
...
def records_table(records: list):
    ...
    for record in records:
        ...
        birthday = ((Fore.GREEN + str(record.birthday))
                    if record.birthday is not None else "---") + Fore.RESET
        phones = ((Fore.YELLOW + record.get_phones())
                  if record.get_phones() else "---") + Fore.RESET
        address = ((Fore.GREEN + str(record.address))
                   if record.address is not None else "---") + Fore.RESET
...
help - print this text with list of commands
